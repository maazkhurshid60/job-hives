        ...
        {categories.map((cat) => (
          <button
            ...
            className={clsx(
              ...
              activeCategory === cat.key && "bg-primary-50 active"
            )}
          >
            <span className={clsx("w-11 h-11 rounded-md flex items-center justify-center flex-shrink-0 cat-ic", cat.bgClass, cat.textClass)}>
              <cat.icon className="w-6 h-6" />
            </span>
            <span className={clsx("flex-1 text-[13.5px] font-semibold cat-name", activeCategory === cat.key ? "text-primary-700 font-bold" : "text-neutral-800")}>
              {cat.label}
            </span>
            <span className={clsx("text-[11.5px] font-bold px-[9px] py-[3px] rounded-full whitespace-nowrap cat-count", activeCategory === cat.key ? "bg-primary-100 text-primary-700" : "bg-neutral-100 text-neutral-500")}>
              {cat.count} Openings
            </span>
          </button>
        ))}
            ...
            {categories.map((cat) => (
              <button
                ...
                className={clsx(
                  ...
                  activeCategory === cat.key && "bg-primary-50 border-primary-300 active"
                )}
              >
                <span className={clsx("w-9 h-9 rounded-md flex items-center justify-center flex-shrink-0", cat.bgClass, cat.textClass)}>
                  <cat.icon className="w-[18px] h-[18px]" />
                </span>
                <span className="text-[12.5px] font-bold text-neutral-800 cat-name">{cat.label}</span>
